...
;; [PM] 2003-04-24 this is rather pointless, could use
;; (file-name-directory load-file-name). Furthermore, if the new
;; qp-setup.el was loaded by ~/.emacs then we do not need to mess with
;; load-path at all.


(if (not (featurep 'qp-setup))          ; 2003-04-24 is not needed if qp-setup has been loaded
    (let ((got (get-load-path command-line-args)))
      (and got                          ; [PM] 2003-04-24 paranoia
           (setq load-path (cons (file-name-directory got)
                                 load-path)))))
;;
...
(require 'qp-setup)                     ; 2003-04-24 Tell Emacs all it needs to know

(require 'qpfindpred    )              ; Contains find definition
					; support fns
(require 'qptokens      )              ; Prolog tokenising stuff
(require 'qprolog-mode  )              ; prolog mode functions
(require 'qprolog-indent)              ; For indentation stuff
(require 'qui_aux       )		; For auxiliary function defns
(require 'qui_cmds      )
(require 'qui_filter    )
...
(defun qui-mode ()
 ...
 \\{qui-mode-map}."
  ...
  (make-local-variable 'indent-line-function) ; for C-j maybe call
  (setq indent-line-function 'prolog-indent-line) ; prolog-mode-variables (BT)
  (setq mode-name "qui")
  (setq major-mode 'qui))
...
(defun padded-length ( string ) 
  (let ((len (length string)))
    (cond ((<= len 9)
	   (concat (int-to-string len) "   ")) ; [PM] 2003-04-24 do not pass integers to concat
	  ((and (<= len 99)
		(> len 9))
	   (concat (int-to-string len) "  ")) ; [PM] 2003-04-24 do not pass integers to concat
	  ((and (<= len 999)
	       (> len 99))
	   (concat (int-to-string len) " ")) ; [PM] 2003-04-24 do not pass integers to concat
	  (t (int-to-string len)))))     ; [PM] 2003-04-24 do not pass integers to concat (return value will be used in concat)
