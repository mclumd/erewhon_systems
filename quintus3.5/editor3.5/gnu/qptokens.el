;;; SCCS: @(#)qptokens.el	76.1 10/28/98
...
; start-of-token returns the point where the next token begins.
; It basically skips white-spaces and comments
(defun start-of-token ()
  (save-excursion
...
; next-token() used to set the mark and then move to the end of the
; token. Now it does not set the mark!
(defun next-token ()
  (let ((char (following-char)) ttype (origpoint (point)))
    (cond 
     ((eobp) (setq ttype "eof"))
     ((and (>= char ?a) (<= char ?z))
      (forward-char)
     ...
     ((and (>= char ?0) (<= char ?9))
      ...
      (if (= (following-char) ?\')
          (let  ((base (string-to-int (buffer-substring origpoint (point))))
		 x)
            (cond 
