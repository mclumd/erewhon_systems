;;; SCCS: @(#)qprolog-indent.el	76.1 10/28/98
      ...
      (cond ((head-of-clause-p)
	    ...
	    (t (let ((indent-point (point)) 
		 ...
		 (cond ((or (nth 3 state) (nth 4 state))
			...
			(cond ((and (setq pos (prolog-inside-term-p))
				    (prev-line-has-comma))
			       (goto-valid-line)
			       (beginning-of-line)
			       (if (equal (prolog-inside-term-p) pos)
				   (get-arg-column (point))
				 (get-arg-column pos)))
			      (t 
	...
	(save-excursion
	  (save-restriction
	    ...
	    (setq bracket-pos (or (scan-lists (point) -1 1) 
				  (point-min)))
	    (cond ((or (= (char-after bracket-pos)	?\()
		       (= (char-after bracket-pos)	?\[))
		   bracket-pos)
		  (t nil))))
	    (error nil))))
...
Returns t unless search stops due to end of buffer."
  ...
  (let ((clause-point (point)) (not-done t) (command-point (point)))
    ...
    (setq clause-point clause-point)	; return clause-point
    )
  )
...
(defun prolog-indent-lines (end)
  (while (<= (point) end)	; rewritten to a while loop (BT)
    (prolog-indent-line)
    (forward-line)))
