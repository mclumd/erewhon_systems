...
:- meta_predicate define_method(+, +, 0).
...
create(Descriptor, Object) :-
	...
	;   var(Descriptor) ->
		raise_exception(instantiation_error(create(Descriptor,Object),
						    1))
	;   raise_exception(domain_error(create(Descriptor,Object),1,
					 create_spec,Descriptor,''))
	).
...
allocate_instance(Descriptor, Object) :-
	...
	(   obj_decl:class_size(Class, Size, Header) ->
		...
		calloc(Fullsize, 1, Address0),
		(   Address0 =:= 0 ->
		 	raise_exception(
				resource_error(create(Descriptor,Object),
					       memory, ''))
		;   true
		),
		put_atom(Address0, 0, Class),
...
send_new_message(Descriptor, Object) :-
	(   on_exception(Error,
			 ...
			 (free_object(Object),raise_exception(Error))) ->
		true
	;   free_object(Object),
	    fail
	).
...
send_new_message1(Descriptor, Object) :-
	...
	;   raise_exception(existence_error(create(Descriptor,Object),1,
...
fast_create(Class, Createmsg, Header, Fullsize, Object) :-
	...
	(   Address0 =:= 0 ->
		...
		raise_exception(resource_error(create(Descriptor,Object),
					       memory, ''))
	;   put_atom(Address0, 0, Class),
	    Address is Address0+Header,
	    functor(Object, Class, 1),
...
%  is multifile.  There is a clause here only to avoid undefined traps.
%  This clause will never be used, because 0 will never unify with an
%  object.

:- multifile class_message_clauses:initialize_slots/1.
class_message_clauses:initialize_slots(0).
...
destroy(Object) :-
	(   var(Object) ->
		raise_exception(instantiation_error(destroy(Object),1))
	;   functor(Object, Class, 1),
	    obj_decl:class_size(Class, _, Header),
	    arg(1, Object, Addr0),
	    integer(Addr0) ->
		Address is Addr0 - Header
	;   raise_exception(domain_error(destroy(Object),1,object,Object,''))
	),
...
class_message_clauses:cleanup_slots(0).
...
class_of(Object, Class) :-
	(   var(Object) ->
		raise_exception(instantiation_error(class_of(Object,Class),1))
	;   functor(Object, Class0, 1),
	    obj_decl:class_size(Class0, _, _) ->
		Class0 = Class
	;   class_of_hook(Object, Class1) ->
		Class = Class1
	;   raise_exception(type_error(class_of(Object, Class), 1,
				       object, Object))
	).
...
descendant_of(Object, Class) :-
	(   var(Object) ->
		raise_exception(instantiation_error(
				   descendant_of(Object,Class),1))
	;   functor(Object, Class0, 1),
	    obj_decl:class_size(Class0, _, _) ->
		true
	;   class_of_hook(Object, Class0) ->
		true
	;   raise_exception(type_error(descendant_of(Object, Class), 1,
				       object, Object))
	),
...
pointer_object(Addr, Obj) :-
	(   integer(Addr) ->
		(   Addr =:= 0 ->
			Obj = null
		;   get_atom(Addr, -4, Functor),
		    functor(Obj, Functor, 1),
		    arg(1, Obj, Addr)
		)
	;   nonvar(Addr) ->
		raise_exception(type_error(pointer_object(Addr,Obj), 1,
					   integer, Addr))
	;   compound(Obj),
	    functor(Obj, _, 1) ->
		arg(1, Obj, Addr)
	;   Obj == null ->
		Addr = 0
	;   var(Obj) ->
		raise_exception(instantiation_error(pointer_object(Addr,Obj),
						    0))
	;   raise_exception(type_error(pointer_object(Addr,Obj), 2,
				       object, Obj))
	).
...
ensure_can_send(Object, Op, Msg) :-
	...
	;   \+ callable(Msg) ->
		Goal =.. [Op,Object,Msg],
		raise_exception(domain_error(Goal,2,valid_message,Msg,''))
...
++description
    ...
    unsigned_integer	a 32 bit unsigned integer
    unsigned_short	a 16 bit unsigned integer
    unsigned_char	an 8 bit unsigned integer
...
define_method(Object, Message, Body) :-
	...
	;   var(Message) ->
		raise_exception(instantiation_error(Goal,2))
	;   var(Body) ->
		raise_exception(instantiation_error(Goal,3))
	;   \+ compound(Object) ->
		raise_exception(type_error(Goal,3,object,Object))
	;   \+ callable(Message) ->
		raise_exception(type_error(Goal,3,valid_message,Message))
	;   \+ callable(Body) ->
		raise_exception(type_error(Goal,3,callable,Body))
	;   class_of(Object, Class),
	...
	;   raise_exception(domain_error(Goal,3,instance_method(Class),
					 Message,''))
	).
...
undefine_method(Object, Messagename, Arity) :-
	...
	;   \+ compound(Object) ->
		raise_exception(type_error(Goal,3,object,Object))
	;   \+ atom(Messagename) ->
		raise_exception(type_error(Goal,3,atom,Messagename))
	;   \+ integer(Arity) ->
		raise_exception(type_error(Goal,3,integer,Arity))
	;   class_of(Object, Class),
	    obj_decl:instance_method(Messagename,Arity,Class,Predname) ->
		Predarity is Arity+2,
		arg(1, Object, Address),
		remove_instance_method(Predname, Predarity, Address)
	;   raise_exception(domain_error(Goal,0,instance_method(Class),
					 Messagename/Arity,''))
	).
...
simple_expand_body(Module:X, _, Class, Object, Address, Msg, X1) :-
	!,
	simple_expand_body(X, Module, Class, Object, Address, Msg, X1).
simple_expand_body(fetch_slot(Slot,Value), _, Class, Object, Address, Msg,
		Goal) :-
	!,
	(   accessor(Class, Slot, Address, Value, Goal, _) ->
		true
	;   raise_exception(domain_error(fetch_slot(Slot,Value), 1,
					 valid_class_slot, Slot,
					 in_method(Class,Object,(<-),Msg)))
	).
simple_expand_body(store_slot(Slot,Value), _, Class, Object, Address, Msg,
		Goal) :-
	!,
	(   accessor(Class, Slot, Address, Value, _, Goal) ->
		true
	;   raise_exception(domain_error(store_slot(Slot,Value), 1,
					 valid_class_slot, Slot,
					 in_method(Class,Object,(<-),Msg)))
	).
...
type_accessor(atom, _, Address, Value,
		objects:get_atom(Address,0,Value),
		objects:put_atom(Address,0,Value)) :-
	!.
...
type_accessor(Type, _, Address, Value, Value is Expr, assign(Expr,Value)) :-
...
user:unknown_predicate_handler(Goal, class_message_clauses,
	...
	atom_chars(Predicate, Predchars),
	message_operator(Op),
	atom_chars(Op, Opchars),
	append(Opchars, Msgchars, Predchars),
	!,
	atom_chars(Msgname, Msgchars),
...
ensure_can_send(Object, Op, Name, Arity) :-
	...
	;   Goal =.. [Op,Object,Msg],
	    ...
	    functor(Msg, Name, Arity),
	    (   var(Object) ->
		    Err = instantiation_error(Goal,1)
	    ;   class_of(Object, Class) ->
		    undefined_msg_exception(Op, Object, Msg, Err)
	    ;   Err = domain_error(Goal, 1, object, Object, '')
	    ),
	    raise_exception(Err)
	).
...
undefined_msg_exception(Op, Object, Message, Err) :-
	...
	class_of(Object, Class),
	G =.. [Op,Object,Message],
	Err = existence_error(G,2,method(Class,Op),Name/Arity,M),
...
get_object(Address, Offset, Value) :-
	Vaddr is address_at(Address+Offset),
	(   Vaddr =:= 0 ->
		Value = null
	;   get_atom(Vaddr,-4,Functor),
...
put_object(null, Address, Offset) :-
	!,
	assign(address_at(Address+Offset), 0).
put_object(Value, Address, Offset) :-
	arg(1,Value,Vaddr),
	assign(address_at(Address+Offset), Vaddr).
...
put_object_debug(Value, Address, Offset, Slot, Class) :-
	...
	;   raise_exception(domain_error(store_slot(Slot,Value), 2,
					 descendant_of(Class),Value,''))
	),
	arg(1,Value,Vaddr),
	assign(address_at(Address+Offset), Vaddr).
...
user:unknown_predicate_handler(class_of_hook(_,_), objects, fail).
...
'QU_messages':typename(create_spec) -->
	['object creation specification'-[]].
'QU_messages':typename(initialization_method) -->
	['initialization method'-[]].
'QU_messages':typename(object) -->
	['object'-[]].
'QU_messages':typename(valid_message) -->
	['valid message'-[]].
'QU_messages':typename(method(Class,Op)) -->
	['for ~q class, message ~w'-[Class,Op]].
'QU_messages':typename(instance_method(Class)) -->
	['instance method for ~q class'-[Class]].
'QU_messages':typename(descendant_of(Class)) -->
...
get_atom/3 and put_atom/3 are foreign predicates, which means
they are quite slow.  In proposal 5525 in the PTG bug system, I've
requested a more efficient way to do this.  If this proposal is
implemented, we should get rid of {get,put}_atom/3 from the
foreign interface below, and all calls to them in this file should be
modified to use the new technology.

===============================================================*/

foreign_file(library(system(structs)),
	     ['_Sget_atom', '_Sput_atom', calloc, free]).

foreign('_Sget_atom', c, get_atom(+address(char),+integer,[-atom])).
foreign('_Sput_atom', c, put_atom(+address(char),+integer,+atom)).

% foreign(malloc, c, malloc(+integer, [-address(opaque)])).
