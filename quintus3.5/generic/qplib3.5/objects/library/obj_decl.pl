...
%  'term_expansion/2' facility.  This allows us to generate efficient
...
reinit_class(Classname) :-
	...
	retractall(dyn_class_message(Classname,_,_,_)),
...
/**
the following is to avoid undefined warnings.  This is unnecessary in
QP 3.1.4 (Sun) and subsequent releases in which declaring a predicate
multifile is sufficient to prevent undefined warnings.
**/

:- multifile user:unknown_predicate_handler/3.
:- discontiguous user:unknown_predicate_handler/3.

user:unknown_predicate_handler(class_size(_,_,_), obj_decl, fail).
user:unknown_predicate_handler(class_file(_,_), obj_decl, fail).
user:unknown_predicate_handler(term_class(_,_,_,_), obj_decl, fail).
user:unknown_predicate_handler(subclass(_,_), obj_decl, fail).
user:unknown_predicate_handler(superclass(_,_,_), obj_decl, fail).
user:unknown_predicate_handler(direct_slot(_,_,_,_,_), obj_decl, fail).
user:unknown_predicate_handler(direct_message(_,_,_,_), obj_decl, fail).
user:unknown_predicate_handler(instance_method(_,_,_,_), obj_decl, fail).
user:unknown_predicate_handler(class_method(_,_,_), obj_decl, fail).
user:unknown_predicate_handler(inherit_from(_,_,_,_,_), obj_decl, fail).
user:unknown_predicate_handler(uninherit_from(_,_,_,_,_), obj_decl, fail).
user:unknown_predicate_handler(inline_method(_,_,_,_,_,_,_), obj_decl, fail).
...
analyze_class_defn(Classdef, Classname, Clauses) :-
	...
	record_direct_slots(Direct_slots, Classname, Ancsize, Size, Attribs0,
			    Clauses2, Clauses3),
	add_bookkeeping_fact(class_size(Classname,Size,4), Clauses3, Clauses4),
...
analyze_class_defn1(Name, [Name|Ps], Ps, Slots, Slots, Clauses,
	...
	;   raise_exception(bad_parent(Name))
	).
...
verify_essence(Essence) :-
	var(Essence),
	!,
	raise_exception(bad_term_class_essence(Essence)).
verify_essence([]) :-
	!.
verify_essence([E|Es]) :-
	!,
	...
	;   raise_exception(bad_term_class_essence(E))
	),
	verify_essence(Es).
verify_essence(Essence) :-
	raise_exception(bad_term_class_essence(Essence)).
...
compile_slot(Name:Type0, Class, Offset0, Next, Name, Type, Attribs0,
	...
	;   compile_slot_type(Type0, Class, Offset1, Attribs0,
			  Type, Offset, Next1, Attribs) ->
		true
	;   raise_exception(bad_slot_type(Type0))
	),
	(   Oldtype \== 0,
	    \+ (Offset=Offset1,compatible_slot_types(Type, Oldtype)) ->
		raise_exception(invalid_slot_shadow(Name,Type,Oldtype))
	;   true
	),
...
begin_pending_class(Classname) :-
	(   var(Classname) ->
		raise_exception(instantion_error((:- class Classname='...'),
						 0))
	;   \+ atom(Classname) ->
		raise_exception(type_error((:- class Classname='...'),0,atom,
			Classname))
	;   x_class_file(Classname, Oldfile),
	    get_current_file(Newfile),
	    Oldfile \== Newfile ->
		raise_exception(permission_error((:- class Classname='...'),
			redefine, class, Classname, ''))
...
handle_address_expressions(Class, Object, Offset, Specs, Goal, Goal0) :-
	...
	handle_address_expressions(Specs, Base).
handle_address_expressions(Class, Object, _, Specs, Goal, Goal) :-
	x_term_class(Class, _, _, _),
	!,
	all_members_equal(Specs, Object).


handle_address_expressions([], _).
handle_address_expressions([Offset-Address|Pairs], Base) :-
	handle_address(Address, Base, Offset),
	handle_address_expressions(Pairs, Base).
...
:- dynamic user_inlineable_parameters/2.
...
:- multifile expand_method_goal/10.
...
accessor(Class, Slot, Self, Op, Msg, Addr, Addr0, Value, Pred, Ctxt,
	...
	(   defined_class(Class),
		...
		(   Class \== Realclass,
			...
			Error = permission_error(Goal,Pred,private_slot,Slot,
					     in_method(Class,Self,Op,Msg)),
			identified_message(warning, Error),
			% Only the right one of these will be used
			Accessor = raise_exception(Error),
			Setter = Accessor
	...
	;   Goal =.. [Pred,Slot,Value],
	    (	var(Slot) ->
		    Error = instantiation_error(Goal,1)
	    ;	Error = domain_error(Goal,1,valid_class_slot,Slot,
				     in_method(Class,Self,Op,Msg))
	    ),
	    identified_message(warning, Error),
	    % Only the right one of these will be used
...
single_accessor(Type, _, expression(A), Value, Value is Expr, 
		assign(Expr,Value)) :-
	simple_accessor(Type, A, Expr),
	!.
single_accessor(atom, _, sum(Address,Offset), Value,
		objects:get_atom(Address,Offset,Value),
		objects:put_atom(Address,Offset,Value)) :-
	!.
...
%  simple_accessor(+Type, +Address, -Expr)
%  Expr is an address expression which may be used in the second
%  argument to is/2 or as the first argument to assign/2 to access or
%  store an element of type Type at address Address.

simple_accessor(integer, Address, integer_at(Address)).
simple_accessor(short, Address, integer_16_at(Address)).
simple_accessor(char, Address, integer_8_at(Address)).
simple_accessor(unsigned_short, Address, unsigned_16_at(Address)).
simple_accessor(unsigned_char, Address, unsigned_8_at(Address)).
simple_accessor(float, Address, single_at(Address)).
simple_accessor(double, Address, double_at(Address)).
simple_accessor(address, Address, address_at(Address)).
...
slot_name(Slot, Name) :-
	...
	;   Slot = [First|Rest],
	    (	Rest == [] ->
		    Name = Slot
	    ;	insert_separators(Rest, Rest2),
		concat_atom([First|Rest2], Name)
	    )
	).
...
insert_separators([], []).
insert_separators([I|Is], ['#',I|Os]) :-
	insert_separators(Is, Os).
...
message_goal(Msg, Obj1, Obj2, Op, Goal) :-
	...
	message_operator(Op),
	Msg =.. [Msgname|Args],
	concat_atom([Op,Msgname], Predname),
...
declare_inheritance(Spec, Clauses, Clauses0) :-
	...
	;   raise_exception(context_error((inherit(Spec),
					   not_in_class_defn,0)))
	).
...
declare_inheritance1(Spec, Class, Clauses, Clauses0) :-
	(   Spec =.. [Op, Parent, Name/Arity],
		...
		declare_inheritance2(Parent, Class, Op, Name, Arity, Spec,
				     Clauses, Clauses0)
	;   identified_message(error, domain_error(inherit(Spec), 1,
					      valid_inherit_spec, Spec, 0))
	).
...
suppress_inheritance(Spec, Clauses, Clauses0) :-
	...
	;   raise_exception(context_error((uninherit(Spec), not_in_class_defn,
					   0)))
	).
...
suppress_inheritance1(Spec, Class, Clauses, Clauses0) :-
	...
	;   identified_message(error, domain_error(uninherit(Spec), 1,
					      valid_inherit_spec, Spec, 0))
	).
...
declare_instance_methods(Name/Arity, Clauses, Clauses0) :-
	...
	add_clause(:-(dynamic(class_instance_method:Predname/Predarity)),
		   Clauses2, Clauses3),
	add_locked_clause(
...
add_locked_clause(Cl, [:-(multifile(Spec)),:-(discontiguous(Spec))|Tail0],
		Tail) :-
	predspec(Cl, Spec),
	prolog_version_supports_locking(Bool),
	(   Bool == true ->
		Tail0 = [:-(locked(Spec)),Cl|Tail]
	;   Tail0 = [Cl|Tail]
	).

:- dynamic supports_locking/1.

prolog_version_supports_locking(Bool) :-
	supports_locking(Bool),
	!.
prolog_version_supports_locking(Bool) :-
        % locked directive only works in QP 3.1.4 and later
	prolog_flag(version, VersionAtom),	
	version_number(VersionAtom, Major, Minor, Micro),
	(   [Major,Minor,Micro] @>= [3,1,4] ->
	        Bool = true
	;   otherwise ->
	        Bool = false
        ),
	assert(supports_locking(Bool)).


%  Silly little routine for taking apart a version number atom, allowing
%  for things like 3.1.4, 3.1.2b, 3.1 and 3.1b.

version_number(VersionAtom, Major, Minor, Micro) :-
	atom_chars(VersionAtom, Chars),
	version_number(Major, Minor, Micro, Chars, _Remainder).

version_number(Major,Minor,Micro) --> 
	int(Major), 
	".", 
	int(Minor), 
	".", 
...
int(I) --> digit(D), int(D, I).

int(D, I) --> digit(D1), !, {I0 is D*10+D1}, int(I0, I).
int(D, D) --> [].

digit(D) --> [Char], {"0"=<Char, Char=<"9", D is Char-"0"}.
...
%  all_members_equal(+-List, +-Elt)
%  Every element of List equals Elt.

all_members_equal([], _).
all_members_equal([Elt|Elts], Elt) :-
	all_members_equal(Elts, Elt).
...
source_term_position(Pos) :-
	(   prolog_load_context(term_position, Pos) ->
		true
	% WARNING:  this hack is only here to allow testing.  It is not 
	%	    really robust, but it's good enough for testing.
	;   testing_position(Pos) ->
		true
	;   Pos = '$stream_position'(0,0,0,0,0)
	).
...
:- dynamic forward_def_slot_type/2.
...
user:unknown_predicate_handler(analyze_class_defn_hook(_,_,_),
			       obj_decl, fail).
...
user:unknown_predicate_handler(analyze_class_defn_hook(_,_,_,_,_,_,_),
			       obj_decl, fail).
...
user:unknown_predicate_handler(compile_slot_type_hook(_,_,_,_,_,_,_,_),
			       obj_decl, fail).


%  slot_type_name(+Type)
%  Type is a valid type name for a slot.  Generally, this predicate should
...
user:unknown_predicate_handler(handle_slot_attrib_hook(_,_,_,_,_,_),
			       obj_decl, fail).
...
user:unknown_predicate_handler(generate_linkage_hook(_,_,_,_,_,_),
			       obj_decl, fail).
...
This code should be removed before this package is wrapped up.
...
% test(Filename) :-
% 	absolute_file_name(Filename, [file_type(prolog)], Fullname),
% 	open(Fullname, read, Stream),
% 	retractall(testing(_)),
% 	assert(testing(Fullname)),
% 	repeat,
% 	    read_term(Stream, [term_position(Pos)], Term),
% 	    retractall(testing_position(_)),	% just in case
% 	    assert(testing_position(Pos)),
% 	    user:term_expansion(Term, Term1),
% 	    retractall(testing_position(_)),
% 	    portray_clauses(Term1),
% 	    Term == end_of_file,
% 	!,
% 	retractall(testing(_)),
% 	close(Stream).
...
% portray_clauses([]) :-
% 	!.
% portray_clauses([Cl|Cls]) :-
% 	!,
% 	portray_clauses(Cl),
% 	portray_clauses(Cls).
% portray_clauses(:-(discontiguous(_))) :- !.
% portray_clauses(:-(multifile(_))) :- !.
% portray_clauses(:-(locked(_))) :- !.
% portray_clauses(X) :-
% 	portray_clause(X).
% 
% 
...
'QU_messages':generate_message(message_at(Message, Position)) -->
	'QU_messages':generate_message(Message),
	'QU_messages':position(Position).
'QU_messages':generate_message(class_decl_error(Decl,Error)) -->
	['In class declaration'-[]],
	[nl],
	['    ~q'-[Decl]],
	[nl],
	'QU_messages':generate_message(Error).
'QU_messages':generate_message(bad_parent(Parent)) -->
	['Invalid parent class ~q:  no such class'-[Parent]],
	[nl].
'QU_messages':generate_message(bad_defn(Spec)) -->
	['Invalid class definition component ~q'-[Spec]],
	[nl].
'QU_messages':generate_message(bad_slot_type(Type)) -->
	['Invalid slot type ~q'-[Type]],
	[nl].
'QU_messages':generate_message(bad_slot_type(Type,InClass)) -->
	['Invalid slot type ~q in class ~q: assuming pointer to ~q'-
	[Type,InClass,Type]],
	[nl].
'QU_messages':generate_message(bad_term_class_essence(Essence)) -->
	['Invalid term class essence specification term:  ~q'-[Essence]],
	[nl].
'QU_messages':generate_message(ambiguous_inheritance(Class,P1,P2,N,A,Op)) -->
	['Ambiguous inheritance in class ~q'-[Class]],
	[nl],
	['method for ~q ~q/~w inherited from ~q and ~q'-[Op,N,A,P1,P2]],
	[nl].
'QU_messages':generate_message(ambiguous_send_super(Goal,Class,P1,P2)) -->
	...
	['Goal:  ~q'-[Goal]],
	[nl].
'QU_messages':generate_message(orphan_send_super(Goal,Class)) -->
	['Invalid send super:  ~q has no superclass'-[Class]],
	[nl],
	['Goal:  ~q'-[Goal]],
	[nl].
'QU_messages':generate_message(invalid_slot_shadow(Name,Type,Type0)) -->
	['Invalid shadowing of inherited slot ~q of type ~q'-[Name,Type0]],
	[nl],
	['with shadowing type ~q'-[Type]],
	[nl].

:- multifile 'QU_messages':contexttype/3.

'QU_messages':contexttype(not_in_class_defn) -->
	['when not in a class definition'-[]].
'QU_messages':contexttype(no_debug_message) -->
	['when debug mode is off'-[]].
'QU_messages':contexttype(not_inherited) -->
	['for message that is not inherited'-[]].


:- multifile 'QU_messages':commandtype/3.

'QU_messages':commandtype(uninherit_directive) -->
	[':- uninherit directive'-[]].
'QU_messages':commandtype(inherit_directive) -->
	[':- inherit directive'-[]].


:- multifile 'QU_messages':typename/3.

'QU_messages':typename(variable) -->
	['unbound variable'-[]].
'QU_messages':typename(valid_class_slot) -->
	['class slot'-[]].
'QU_messages':typename(valid_inherit_spec) -->
	['inheritance specification term'-[]].
'QU_messages':typename(class) -->
	[('class')-[]].
'QU_messages':typename(ancestor_of(Class)) -->
	['ancestor of ~w class'-[Class]].
'QU_messages':typename(private_slot) -->
	['private slot'-[]].


:- multifile 'QU_messages':operation/3.

'QU_messages':operation(fetch_slot) -->
	['fetch'-[]].
'QU_messages':operation(store_slot) -->
	['store into'-[]].
...
'QU_messages':msg(in_method(Class,_,Op,Message)) -->
	{functor(Message,Name,Arity)},
...
:- dynamic current_clause_position/1.
:- dynamic inline_method_pos/5.
...
:- use_module(library(expansion), [
        clause_expansion_loop/3
   ]).

:- multifile user:clause_expansion/3.

user:clause_expansion(Clauses0, Tags, Clauses) :-
	nonmember(class_decl_expansion, Tags),
	class_decl_expansion(Clauses0, Clauses1),
	clause_expansion_loop(Clauses1, [class_decl_expansion|Tags], Clauses).

%% Adding license check
% :-initialization  '$Quintus: check_license'(objects, '3.5').
