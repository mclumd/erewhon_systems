...
read_clauses(S, Pred, Defaultmod, Modname, File, Clauses) :-
    ...
    (   Clause0 = :-(module(Modname,Exports)) ->
	    assert(determinacy:module_exports(Modname, Exports)),
	    assert(determinacy:file_processed(File, Modname)),
...
% 	    read_clauses1(S, Modname, Pred, File, Clauses)
    ...
    ;       assert(determinacy:file_processed(File, Defaultmod)),
	    fail
    ).
...
/* ----------------------------------------------------------------------
	...
	% qld [-D] -dvo qpdet det.qof 'messages(language(QU_messages))'
 
 	% qpdet file1 file2 ...

   ---------------------------------------------------------------------- */

%- use_module(messages(language('QU_messages'))).   % needed for runtime 
						   % system only
...
'usage: qpdet [-r] [-d] [-D] [-i ifile] fspec ...
