/* ----------------------------------------------------------------------
    File:	xref.pl
    Authors:	Tom Howland and Peter Schachte
    SCCS:	%W% %G%
    Purpose:	Cross referencer
    ...
    This file contains the Quintus cross referencer.
   ---------------------------------------------------------------------- */
...
xref(Files, Options) :-
    ...
    prolog_flag(gc_margin, Old, 9216),	% QP 9 times the default,
    init_d(Dict0),
    ...
    prolog_flag(gc_margin, _, Old), % QP
...
load_files(X, Options, Mod, F, Level-Verbose, D0, D) :-
    fparse(X, F, Z),
    absolute_file_name(Z, [file_type(prolog),access(exist),file_errors(fail)],
		       File), !,
    open(File, read, Stream),
    read_one(Stream, Term, LC),
    (   Term = (:-module(M,Exports))
...
% recognize(foreign_resource(_,_), _, _, _, _) --> !. % SP
recognize([], _,  _, _, _) --> !.
...
command(multifile(Dynamic), LC, Mod, File, _) --> !,
...
% dynamic(foreign_resource, 2, _, _, _) --> !. % SP
dynamic(N, A, LC, Mod, File) -->
	{atom(N), integer(A)},
...
% body(undo(X), M)                 --> !, parent(undo, 1, X, M).         % SP
...
body(on_exception(_,X,Y), M0)    --> !, {parent(on_exception, 3, M0, M)},
    body(X, M), body(Y, M).
% body(catch(_,X,Y), M0)           --> !, body(on_exception(X,_,Y), M0).	% SP
...
% referencing is then done by following user:runtime_entry/1, externs, and
...
follow_top(TraceRoot, Imports, Externs, INI) -->
    ...
    follow_hooks([% query_class/5, % SP
		  % query_input/3, % SP
		  % query_map/4, % SP
		  generate_message/3, % SP/QP
...
undefined_ok(parse_message, 3, 'QU_messages'). % QP
...
fetchm(K,  A, E, I, A) :- avl_fetch(K,  A, E-I), !.
...
print_warning(extern(N,A,M)) :-
    write(' extern declaration for undefined predicate '),
    write_pred_spec(M, N, A).
...
'usage: qpxref [-R] [-c] [-v] [-i ifile] [-w wfile] [-x xfile] [-m ufile] [-u ufile] fspec ...
