...
get_option(process_dependency, M:Pred) :-
	clause(process_stream(_,_,_,_), M:Body),
	functor(Pred, 4, Body).
...
recover_error(existence_error(read(_,_),_,_,_,_), _, end_of_file) :- !. % QP
% recover_error(existence_error(read(_,_),_,_,_,past_end_of_stream), _, end_of_file) :- !. % SP
...
process_dependency(File1, Mod, Imports, Where-Level) :-
    fparse(File1, Where, File2),
    absolute_file_name(File2, [file_type(prolog),access(exist),file_errors(fail)],
		       Path), !,
...
%     !,
%     calls_known_nondet(Goal, Mod, Pred).
calls_known_nondet((_^Goal), Mod, Pred) :- % QP/SP
    !,
    calls_known_nondet(Goal, Mod, Pred).
...
calls_known_nondet(absolute_file_name(_,Opts,_), _, absolute_file_name/3) :- % QP
    !,
    ground(Opts),
...
% hook(query_class_hook(_,_,_,_,_), user). % SP
% hook(query_input_hook(_,_,_), user). % SP
% hook(query_map_hook(_,_,_,_), user). % SP
hook(unknown_predicate_handler(_,_,_), user).
hook(user_help, user).
hook(term_expansion(_,_), user).
% hook(term_expansion(_,_,_,_), user). % SP
% hook(goal_expansion(_,_,_), user). % SP
% hook(debugger_command_hook(_,_), user). % SP
% hook(error_exception(_), user). % SP
% hook(portray_message(_,_), user). % SP
hook(file_search_path(_,_), user).
hook(library_directory(_), user).
hook(generate_message(_,_,_), 'QU_messages').
hook(query_abbreviation(_,_,_), 'QU_messages').
% hook(query_class(_,_,_,_,_), 'QU_messages'). % SP
% hook(query_input(_,_,_), 'QU_messages'). % SP
% hook(query_map(_,_,_,_), 'QU_messages'). % SP
...
print_error_message(syntax_error(_,Pos,Msg,Left,Right,Fl)) :-
    !,
    print_message(error, syntax_error(0,Pos,Msg,Left,Right,Fl)).
...
%- dynamic   user:generate_message_hook/3. % SP
