%   Package: xml.pl
%   Author : Binding Time Ltd.
%   Updated: November 2003
%   Purpose: Parse and generate XML
...
% minimally adapted for Quintus Prolog
% Mats Carlsson, 2003
...
xml_parse( Chars, Document, Controls ) :-
	...
	must_be(proper_list, Controls, 3, Goal),
...
xml_options([X|L], Opt0, Opt, Goal, ArgNo) :-
	(   nonvar(X),
        ...
        ;   must_be(xml_parse_option, X, ArgNo, Goal)
        ),
...
fault_id( Attributes ) -->
	{member( id=Chars, Attributes ), chars( Chars )},
	!,
	"(", chars(Chars), ")".
 ...
 * History:
 * $Log: xml.pl,v $
 * Revision 1.1  2003/11/17 17:00:04  matsc
 * library(xml) added
 *
 * Revision 1.1  2003/09/26 12:24:05  matsc
 * library(xml)
 *
...
'QU_messages':typename(xml_parse_option) --> ['XML parse option'-[]].
