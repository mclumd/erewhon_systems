...
foreign_file('BSDcurses.o',[initscr, p_clear, p_refresh, endwin,
	...
	gettmode, mvcur, scroll, setterm, tstp, 
...
foreign(p_getyx, getyx(+integer,+integer,+integer)).
foreign(p_inch, inch).
...
foreign(wmove, wmove(+integer,+integer,+integer)).
...
foreign(p_mvaddch, mvaddch(+address,+integer,+integer)).
foreign(p_mvgetch, mvgetch(+address,+integer,[-integer])).
foreign(p_mvaddstr, mvaddstr(+address,+integer,+string)).
foreign(p_mvgetstr, mvgetstr(+address,+integer,-string)).
foreign(p_mvinch, mvinch(+address,+integer,[-integer])).
foreign(p_mvdelch, mvdelch(+address,+integer)).
foreign(p_mvinsch, mvinsch(+address,+integer,+integer)).
...
:- load_foreign_files(['BSDcurses.o'],['-lg', '-lcurses', '-ltermcap' ]),
