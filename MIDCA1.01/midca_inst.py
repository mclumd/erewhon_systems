...
def run_regular(predefgoals = False):
	...
	if predefgoals:
		midca1.assign_module("Goal Selection", goalgen.rungen.StoredGoalLoader())
	midca1.init()
...
def run_arson():
	...
	while 1:
		val = raw_input()
		if val == "q":
			#tell Meta-Aqua to shut down
			writeSocket = midca1.mem.get(SOCKET_W)
			writeSocket.send("((Done))")
			readSocket = midca1.mem.get(SOCKET_R)
...
if __name__ == "__main__":
	if raw_input("Press 1 for default fire scenario; any other value for arsonist scenario.") == "1":
		if raw_input("Press 1 for predefined goals") == "1":
			run_regular(True)
		else:
			run_regular()
	else:
