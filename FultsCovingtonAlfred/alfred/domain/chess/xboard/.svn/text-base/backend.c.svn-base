 ...
 * XBoard $Id: backend.c,v 1.1 2004/06/17 22:04:52 darsana Exp $
 *
     ...
     ChessProgramState *cps;
{
    ...
    if (outCount < count && !exiting) {//RANDY - Socket problem here.
	sprintf(buf, "Error writing to %s chess program", cps->which);
	printf("(message,outCount,count,exiting,cps->pr)=(%s,%d,%d,%d,%d)\n",message,outCount,count,exiting,cps->pr);
