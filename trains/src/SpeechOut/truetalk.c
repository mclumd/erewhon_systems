 ...
 * Time-stamp: <96/08/14 15:43:37 ferguson>
 *
...
static void ttIndexCB(TTclient client, TTevent event, int value);
...
truetalkInit(char *host)
{
    ...
    /* Setup a callback for index markers */
    TT_SetEventCallback(myclient, TT_Index, ttIndexCB);
    /* Done */
    DEBUG0("done");
    return 0;
}
...
static void
ttIndexCB(TTclient client, TTevent event, int value)
{
    DEBUG1("value=%d", value);
    sendIndexReply(currentPerf, value);
    DEBUG0("done");
}
