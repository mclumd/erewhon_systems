;;;
;;; dump-psm.lisp : Whatever is necessary to dump and restart an image
;;;
;;; George Ferguson, ferguson@cs.rochester.edu, 17 Dec 1995
;;; Time-stamp: <Mon Jan 20 16:54:09 EST 1997 ferguson>
;;;
...
(defvar *psm-image-name* "tpsm")

;; Name of function to call when dumped lisp restarts
(defvar *psm-restart-function* 'kqml-psm)
...
;; added.
;;
(defun dump-psm (&optional (image-name "tpsm"))
  "Dump a new Lisp image named IMAGE-NAME containing the psm. "
...
#-(version>= 4 3)
  ...
  (excl:gc nil)
  (excl:gc t)
  ;; Dump the image, which will restart by calling *psm-restart-function*
...
It simply calls the value of *psm-restart-function* inside a macro that
causes a stack trace (to *error-output*) on error."
  ...
  ;; Disable debugging (or something like that) in dumped lisp
  (setq *PSM-interruptable* nil)
  ;; Then run the restart function wrapped in the zoom-maker
  (let ((*log-stream* (open "ps.log" :direction :output
			    :if-exists :supersede
			    :if-does-not-exist :create)))
    (handler-bind
	((error #'(lambda (e)
		    (zoom-and-exit e))))
      (apply *psm-restart-function* nil))))

(defun zoom-and-exit (e)
  "Called after error E has been trapped to cause a stack trace. It uses
*log-stream* if it is non-nil, otherwise writes to the file ps.zoom.
It then calls excl:exit to terminate execution (and so never returns)."
  (format *error-output* "PS Error: ~A (see ps.log or ps.zoom)~%" e)
