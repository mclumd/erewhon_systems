(in-package parser)
;;Time-stamp: <96/01/23 18:18:41 miller>
...
(defun init-spellchecklist-to-cities nil
  (reset-wordlist
   (union (mapcar 'car *lexicon-city*)
	  (reduce-to-single-words
	   (mapcar 'car *lexicon-city*)))))
...
(let ((wordlist nil)
  ...
  (defun speeling-corrector (token &optional (threshold 100))
	...
	(let* ((matchlist (find-match-to-wordlist token wordlist))
	       (smatch (find-max matchlist :key #'car)))
...
(defun find-max (l &key (key #'identity))
  "find the maximum entry on l, using key if needed."
  (let ((max-so-far (car l)))
    (find-max-i (cdr l) max-so-far key)))

(defun find-max-i (l max-so-far key)
  (cond
   ((null l)
    max-so-far)
   ((> (funcall key (car l))
       (funcall key max-so-far))
    (find-max-i (cdr l) (car l) key))
   (t
    (find-max-i (cdr l) max-so-far key))))
