...
(defun find-right-type (ccas)
  (cond ((not ccas) nil)
	((eq (length (CCA-acts (first ccas))) 1)
...
(defun adjust (cca n)
  (let ((sum 0))
    (dolist (item (CCA-acts (rest cca)) sum)
      (setf sum (+ sum (SA-reliability item))))
    (setf (CCA-reliability (rest cca)) (float (/ sum(length (CCA-acts (rest cca)))))))
  (cons (cons n (first cca)) (rest cca)))
...
(defun varp (term)
  (and (keywordp term) (eq (char (string term) 0) #\V)))
