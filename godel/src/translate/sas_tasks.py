...
class SASTask:
    ...
    def output(self, stream):
        ...
        for axiom in self.axioms:
            axiom.output(stream)
        print(len(self.methods), file=stream)
...
class SASMethod:
    def __init__(self, name, goal, prevail, subgoals):
        self.name = name
        self.goal = goal
    ...
    def dump(self):
        ...
        for subgoal in self.subgoals:
            print ("-- Subgoal %d:" % count)
            for var, val in subgoal.pairs:
                print ("---- v%d: %d" % (var, val))
            count += 1

    def output (self, stream):
        print ("begin_method", file=stream)
        print (self.name, file=stream)
        print (len(self.goal.pairs), file=stream)
        for var, val in self.goal.pairs:
            print ("%d %d" % (var, val), file=stream)
        print (len(self.prevail), file=stream)
        for var, val in self.prevail:
            print ("%d %d" % (var, val), file=stream)
        print (len(self.subgoals), file=stream)
        for subgoal in self.subgoals:
            print (len(subgoal.pairs), file=stream)
            for var, val in subgoal.pairs:
                print ("%d %d" % (var, val), file=stream)
        print ("end_method", file=stream)
