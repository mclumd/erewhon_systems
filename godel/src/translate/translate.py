...
def translate_strips_operator_aux(operator, dictionary, ranges, mutex_dict,
    ...
    print ("inside TSOA ... operator name is %s" % operator.name)
    ...
    for conditions, fact in operator.add_effects:
        ...
        for var, val in dictionary[fact]:
            print ("for fact %s, var is %d, val is %d" % (fact, var, val))
...
def translate_strips_method (method, dictionary, ranges, mutex_dict, mutex_ranges):
    print ("In translate_strips_method:")
    print (method.name)

    def strips_conjunction_to_bool_list (conjunction, dictionary, ranges, 
                                         mutex_dict, mutex_ranges):
        result = translate_strips_conditions(conjunction, dictionary, ranges, 
                                             mutex_dict, mutex_ranges)
        result_list = [list(result_part.items()) for result_part in result]
        return result_list[0]

    goal_list = strips_conjunction_to_bool_list (method.goal, dictionary, ranges, 
                                                 mutex_dict, mutex_ranges)
    sas_goal = sas_tasks.SASGoal (goal_list)

    sas_prevail = strips_conjunction_to_bool_list (method.precondition, dictionary, 
                                                    ranges, mutex_dict, mutex_ranges)

    sas_subgoals = []
    for subgoal in method.subgoals:
        subgoal_list = strips_conjunction_to_bool_list (subgoal, dictionary, ranges, 
                                                 mutex_dict, mutex_ranges)
        sas_subgoal = sas_tasks.SASGoal (subgoal_list)
        sas_subgoals.append(sas_subgoal)

    return sas_tasks.SASMethod (method.name, sas_goal, sas_prevail, sas_subgoals)
...
def translate_strips_methods (methods, strips_to_sas, ranges, mutex_dict, mutex_ranges):
    result = []
    for method in methods:
        sas_method = translate_strips_method (method, strips_to_sas, ranges, mutex_dict, mutex_ranges)
        result.append (sas_method)
    return result

def translate_task(strips_to_sas, ranges, translation_key,
                   mutex_dict, mutex_ranges, mutex_key,
                   init, goals,
                   actions, axioms, methods, metric, implied_facts):
    ...
    methods = translate_strips_methods (methods, strips_to_sas, ranges, mutex_dict, mutex_ranges)

    ## print ("SAS methods are:")
    ## for m in methods:
    ##     m.dump()
    ...
    return sas_tasks.SASTask(variables, mutexes, init, goal,
                             operators, axioms, methods, metric)
...
def pddl_to_sas(task):
    with timers.timing("Instantiating", block=True):
        (relaxed_reachable, atoms, actions, axioms,
         methods, reachable_action_params) = instantiate.explore(task)

    print ("number of facts in pddl_to_sas is %s\n" % len(atoms))

    ## for m in methods:
    ##     m.dump()

    # print ("in relaxed analysis, reachable atoms are %s \n reachable actions are %s\n" % (atoms, actions))
    ...
    ## print ("dictionary:")
    ## for i, j in strips_to_sas.iteritems():
    ##     i.dump()
    ##     print (j)
    ...
    print ("mutexes:")
    print (len(mutex_ranges))
    print ("mutex dict:")
    for i, j in mutex_dict.iteritems():
        i.dump()
        print (j)
    ...
    with timers.timing("Translating task", block=True):
        sas_task = translate_task(
            ...
            mutex_dict, mutex_ranges, mutex_key,
            task.init, goal_list, actions, axioms, methods, task.use_min_cost_metric,
            implied_facts)
    
    ## print ("SAS operators look like this:")
...
if __name__ == "__main__":
    ...
    print("\nfinished opening pddl file ...\n")

    print("just to verify what the task object contains ... \n")
    task.dump()
