...
class MethodConditionProxy (ConditionProxy):
    def __init__(self, method, postcondition):
        self.owner = method
        self.condition = method.precondition
        self.post = postcondition
    def build_rules(self, rules):
        method = self.owner
        rule_head = get_action_predicate(method)
        rule_body = list(condition_to_rule_body(method.parameters,
                                                self.condition))
        # now rule_body contains the preconditions of the method. need to
        # add the goal predicate in the form of '<name>-is-goal'
        
        ### the next line is required for controlled grounding!!
        rule_body.append (pddl.Atom(self.post.predicate + "-is-GOAL", self.post.args))
        print ("In MCP, rule_body is %s, rule_head is %s" % (rule_body, rule_head))
        rules.append((rule_body, rule_head))

class MethodSubgoalProxy (ConditionProxy):
    def __init__(self, method, subgoal):
        self.owner = method
        self.subgoal = subgoal
        self.condition = pddl.Truth()
    def build_rules(self, rules):
        method = self.owner
        subgoal = self.subgoal
        rule_body = [get_action_predicate(method),]
        rule_head = pddl.Atom(subgoal.predicate + "-is-GOAL", subgoal.args)
        # now rule_body contains the preconditions of the method. need to
        # add the goal predicate in the form of '<name>-is-goal'
        print ("In MSP, rule_body is %s, rule_head is %s" % (rule_body, rule_head))
        rules.append((rule_body, rule_head))
...
def all_conditions(task):
    ...
    # added for GDP ... create conditions for each method
    # for m=(goal, pre, subgoals), create rules of the form:
    #   pre /\ goal-f -> m-applicable ... for all f \in goal
    #   m-applicable -> goal-g ... forall g \in subgoals
    test_rules = []
    for method in task.methods:
        goal = method.goal
        # make the goal list iterable
        if isinstance(goal, pddl.Conjunction):
            parts = goal.parts
        else:
            parts = (goal,)
        # now, for each part of the goal, create a new rule
        for part in parts:
            proxy = MethodConditionProxy(method, part)
            yield proxy
            # proxy.build_rules(test_rules)
        # now that method applicability is determined, add the subgoals
        # as possible goals to achieve, thus making more methods applicable
        # later
        union = subgoals_union (method.subgoals)
        # print ("union of subgoals is %s" % union)
        for subgoal in union :
            # print ("subgoal is %s" % subgoal)
            proxy = MethodSubgoalProxy(method, subgoal)
            yield proxy
            # proxy.build_rules(test_rules)
        # print ("union of subgoals is %s" % union)
        ...
        # for condition, effect in test_rules:
        #    print(pddl_to_prolog.Rule(condition, effect))
...
# takes a list of Condition objects and returns their union
def subgoals_union (subgoals):
    union = set()
    for subgoal in subgoals:
        for part in condition_to_list(subgoal):
            union.add(part)

    return union

def condition_to_list (condition):
    if isinstance(condition, pddl.Conjunction):
