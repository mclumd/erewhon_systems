...
class Task(object):
    def __init__(self, domain_name, task_name, requirements,
                 types, objects, predicates, functions, init, goal, actions, axioms, methods, use_metric):
    ...
    def parse(domain_pddl, task_pddl):
        domain_name, domain_requirements, types, constants, predicates, functions, actions, axioms, methods \
        ...
        return Task(domain_name, task_name, requirements, types, objects,
                    predicates, functions, init, goal, actions, axioms, methods, use_metric)
...
def parse_domain(domain_pddl):
    print ("domain_pddl is %s " %domain_pddl)
    ...
    for entry in entries:
        if entry[0] == ":derived":
            ...
            method = methods.Method.parse(entry)
            the_methods.append(method)
        else:
            action = actions.Action.parse(entry)
            the_actions.append(action)
    
    #print ("parsed actions look like: %s" %the_actions)
    
    # testing whether methods are stored correctly
    #for m in the_methods:
    #    m.dump()
    #print ("parsed methods look like: %s" %the_methods)
    yield the_actions
    yield the_axioms
    yield the_methods
