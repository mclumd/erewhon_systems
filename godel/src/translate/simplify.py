...
class VarValueRenaming(object):
    ...
    def apply_to_task(self, task):
        print ("new_var_nos is ")
        for i in self.new_var_nos:
            print (i)
        ...
        self.apply_to_methods(task.methods)
    ...
    def apply_to_methods(self, methods):
        new_methods = []
        num_removed = 0

        for m in methods:
            ## print ("\nbefore simplifying, method was as follows:")
            ## m.dump()
            try:
                self.apply_to_method(m)
                new_methods.append(m)
                ## print ("\nafter simplifying, method was as follows:")
                ## m.dump()
            except (Impossible, DoesNothing):
                num_removed += 1
                if DEBUG:
                    print ("Removed method: %s" % m.name)
        print ("%d methods removed" % num_removed)
        methods[:] = new_methods
    ...
    def apply_to_method(self, m):
        self.translate_pairs_in_place(m.goal.pairs)
        print ("translaing prevail of methods now ...")
        self.translate_pairs_in_place(m.prevail)
        for subgoal in m.subgoals:
            self.translate_pairs_in_place(subgoal.pairs)
