...
void HMHeuristic::init_hm_table(tuple_vec &t) {
    map<tuple_vec, int>::iterator it;
    for (it = hm_table.begin(); it != hm_table.end(); it++) {
        pair<tuple_vec, int> hm_ent = *it;
        tuple_vec &tup = hm_ent.first;
...
void HMHeuristic::extend_tuple(tuple_vec &t, const Operator &op) {
    map<tuple_vec, int>::const_iterator it;
    for (it = hm_table.begin(); it != hm_table.end(); it++) {
        pair<tuple_vec, int> hm_ent = *it;
        tuple_vec &entry = hm_ent.first;
...
int HMHeuristic::eval(tuple_vec &t) {
    vector<tuple_vec> partial;
...
int HMHeuristic::update_hm_entry(tuple_vec &t, int val) {
...
void HMHeuristic::generate_all_tuples(int var, int sz, tuple_vec &base) {
    if (sz == 1) {
...
void HMHeuristic::generate_all_partial_tuple(tuple_vec &base_tuple, tuple_vec &t,
                                             int index, int sz, vector<tuple_vec> &res) {
    if (sz == 1) {
...
int HMHeuristic::check_tuple_in_tuple(const tuple_vec &tup, const tuple_vec &big_tuple) {
