...
typedef vector<pair<int, int> > tuple_vec;
...
protected:
    ...
    // h^m table
    map<tuple_vec, int> hm_table;
    bool was_updated;

    void init_hm_table(tuple_vec &t);
    void update_hm_table();
    int eval(tuple_vec &t);
    int update_hm_entry(tuple_vec &t, int val);
    void extend_tuple(tuple_vec &t, const Operator &op);

    // some helper methods
    int check_tuple_in_tuple(const tuple_vec &tup, const tuple_vec &big_tuple);
    void state_to_tuple(const State &state, tuple_vec &t) {
    ...
    void get_operator_pre(const Operator &op, tuple_vec &t) {
        for (int i = 0; i < op.get_prevail().size(); i++)
    ...
    void get_operator_eff(const Operator &op, tuple_vec &t) {
        for (int i = 0; i < op.get_pre_post().size(); i++)
    ...
    void generate_all_tuples(int var, int sz, tuple_vec &base);

    void generate_all_partial_tuple(tuple_vec &base_tuple, vector<tuple_vec> &res) {
        tuple_vec t;
        generate_all_partial_tuple(base_tuple, t, 0, m, res);
    }
    void generate_all_partial_tuple(tuple_vec &base_tuple, tuple_vec &t, int index, int sz, vector<tuple_vec> &res);

    void dump_table() const {
        map<tuple_vec, int>::const_iterator it;
        for (it = hm_table.begin(); it != hm_table.end(); it++) {
            pair<tuple_vec, int> hm_ent = *it;
            cout << "h[";
            print_tuple(hm_ent.first);
            cout << "] = " << hm_ent.second << endl;
        }
    }
    void print_tuple(tuple_vec &tup) const {
