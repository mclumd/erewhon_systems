#include "landmark_factory_gdp.h"
...
LandmarkFactoryGDP::LandmarkFactoryGDP(const Options &opts)
    : lm_graph(new LandmarkGraphGDP(opts)) {
}

LandmarkGraphGDP *LandmarkFactoryGDP::compute_lm_graph() {
...
void LandmarkFactoryGDP::generate() {
...
bool LandmarkFactoryGDP::achieves_non_conditional(const Operator &o,
...
bool LandmarkFactoryGDP::is_landmark_precondition(const Operator &o,
                                               const LandmarkNode *lmp) const {
...
bool LandmarkFactoryGDP::relaxed_task_solvable(vector<vector<int> > &lvl_var,
    ...
    for (int i = 0; i < (*g_current_goal).size(); i++)
        if (lvl_var[(*g_current_goal)[i].first][(*g_current_goal)[i].second] ==
            numeric_limits<int>::max())
            return false;
...
bool LandmarkFactoryGDP::is_causal_landmark(const LandmarkNode &landmark) const {
...
bool LandmarkFactoryGDP::effect_always_happens(const vector<PrePost> &prepost, set<
...
bool LandmarkFactoryGDP::interferes(const LandmarkNode *node_a,
...
void LandmarkFactoryGDP::approximate_reasonable_orders(bool obedient_orders) {
    ...
    */
    for (set<LandmarkNode *>::iterator it = lm_graph->get_nodes().begin(); it != lm_graph->get_nodes().end(); it++) {
        ...
        if (node_p->is_true_in_state(*g_current_state))
            return;
...
void LandmarkFactoryGDP::collect_ancestors(
...
void LandmarkFactoryGDP::edge_add(LandmarkNode &from, LandmarkNode &to,
                               edge_type type) {
...
void LandmarkFactoryGDP::discard_noncausal_landmarks() {
...
void LandmarkFactoryGDP::discard_disjunctive_landmarks() {
...
void LandmarkFactoryGDP::discard_conjunctive_landmarks() {
...
void LandmarkFactoryGDP::discard_all_orderings() {
...
void LandmarkFactoryGDP::mk_acyclic_graph() {
...
bool LandmarkFactoryGDP::remove_first_weakest_cycle_edge(LandmarkNode *cur,
...
int LandmarkFactoryGDP::loop_acyclic_graph(LandmarkNode &lmn, hash_set<
...
int LandmarkFactoryGDP::calculate_lms_cost() const {
    int result = 0;
    for (set<LandmarkNode *>::const_iterator it = lm_graph->get_nodes().begin(); it
         != lm_graph->get_nodes().end(); it++)
        result += (*it)->min_cost;
...
void LandmarkFactoryGDP::compute_predecessor_information(
    LandmarkNode *bp,
...
void LandmarkFactoryGDP::calc_achievers() {
