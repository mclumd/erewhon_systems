#include "landmark_factory_gdp_rpg_sasp.h"
...
LandmarkFactoryGDPRpgSasp::LandmarkFactoryGDPRpgSasp(const Options &opts)
    : LandmarkFactoryGDP(opts) {
}

void LandmarkFactoryGDPRpgSasp::get_greedy_preconditions_for_lm(
        ...
        if (prepost[j].pre != -1) {
            ...
            for (int i = 0; i < lmp->vars.size(); i++) {
                if (lmp->vars[i] == prepost[j].var
                    && (*g_current_state)[prepost[j].var] != lmp->vals[i]) {
                    result.insert(make_pair(prepost[j].var,
                                            (*g_current_state)[prepost[j].var]));
                    break;
                }
            }
        }
...
int LandmarkFactoryGDPRpgSasp::min_cost_for_landmark(LandmarkNode *bp, vector<vector<
                                                                               int> > &lvl_var) {
...
void LandmarkFactoryGDPRpgSasp::found_simple_lm_and_order(const pair<int, int> a,
...
void LandmarkFactoryGDPRpgSasp::found_disj_lm_and_order(const set<pair<int, int> > a,
                                                     LandmarkNode &b, edge_type t) {
    bool simple_lm_exists = false;
    pair<int, int> lm_prop;
    for (set<pair<int, int> >::iterator it = a.begin(); it != a.end(); ++it) {
        if ((*g_current_state)[it->first] == it->second) {
...
void LandmarkFactoryGDPRpgSasp::compute_shared_preconditions(
...
void LandmarkFactoryGDPRpgSasp::build_disjunction_classes() {
...
void LandmarkFactoryGDPRpgSasp::compute_disjunctive_preconditions(vector<set<pair<int,
...
void LandmarkFactoryGDPRpgSasp::generate_landmarks() {
    cout << "Generating landmarks using the RPG/SAS+ approach\n";
    build_disjunction_classes();

    for (unsigned i = 0; i < (*g_current_goal).size(); i++) {
        LandmarkNode &lmn = lm_graph->landmark_add_simple((*g_current_goal)[i]);
        lmn.in_goal = true;
    ...
    while (!open_landmarks.empty()) {
        ...
        if (!bp->is_true_in_state(*g_current_state)) {
            ...
            for (int i = 0; i < disjunctive_pre.size(); i++)
                // TODO: Edited just to see if we're missing any important lms because of this condition
...
void LandmarkFactoryGDPRpgSasp::approximate_lookahead_orders(
...
bool LandmarkFactoryGDPRpgSasp::domain_connectivity(const pair<int, int> &landmark,
     ...
     */
    assert(landmark.second != (*g_current_state)[landmark.first]); // no initial state landmarks
    ...
    if (exclude.find((*g_current_state)[landmark.first]) != exclude.end())
        return false;
    list<int> open;
    hash_set<int> closed(g_variable_domain[landmark.first]);
    closed = exclude;
    open.push_back((*g_current_state)[landmark.first]);
    closed.insert((*g_current_state)[landmark.first]);
...
void LandmarkFactoryGDPRpgSasp::find_forward_orders(
    const vector<vector<int> > &lvl_var, LandmarkNode *lmp) {
...
void LandmarkFactoryGDPRpgSasp::add_lm_forward_orders() {
