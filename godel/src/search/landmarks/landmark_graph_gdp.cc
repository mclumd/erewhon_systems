...
LandmarkGraphGDP::LandmarkGraphGDP(const Options &opts)
    : exploration(opts.get<ExplorationGDP *>("explor")),
...
void LandmarkGraphGDP::generate_operators_lookups() {
...
LandmarkNode *LandmarkGraphGDP::get_landmark(const pair<int, int> &prop) const {
...
LandmarkNode *LandmarkGraphGDP::get_lm_for_index(int i) {
    assert(ordered_nodes[i]->get_id() == i);
    return ordered_nodes[i];
}

int LandmarkGraphGDP::number_of_edges() const {
    int total = 0;
...
void LandmarkGraphGDP::count_costs() {
    reached_cost = 0;
    needed_cost = 0;
...
bool LandmarkGraphGDP::simple_landmark_exists(const pair<int, int> &lm) const {
    hash_map<pair<int, int>, LandmarkNode *, hash_int_pair>::const_iterator it =
...
bool LandmarkGraphGDP::landmark_exists(const pair<int, int> &lm) const {
...
bool LandmarkGraphGDP::disj_landmark_exists(const set<pair<int, int> > &lm) const {
...
bool LandmarkGraphGDP::exact_same_disj_landmark_exists(const set<pair<int, int> > &lm) const {
...
LandmarkNode &LandmarkGraphGDP::landmark_add_simple(const pair<int, int> &lm) {
...
LandmarkNode &LandmarkGraphGDP::landmark_add_disjunctive(const set<pair<int, int> > &lm) {
    vector<int> vars;
    vector<int> vals;
...
LandmarkNode &LandmarkGraphGDP::landmark_add_conjunctive(const set<pair<int, int> > &lm) {
    vector<int> vars;
    vector<int> vals;
    for (set<pair<int, int> >::iterator it = lm.begin(); it != lm.end(); ++it) {
        vars.push_back(it->first);
...
void LandmarkGraphGDP::rm_landmark_node(LandmarkNode *node) {
...
LandmarkNode &LandmarkGraphGDP::make_disj_node_simple(pair<int, int> lm) {
...
void LandmarkGraphGDP::set_landmark_ids() {
...
void LandmarkGraphGDP::dump_node(const LandmarkNode *node_p) const {
...
void LandmarkGraphGDP::dump() const {
...
void LandmarkGraphGDP::add_options_to_parser(OptionParser &parser) {
...
void LandmarkGraphGDP::dump_landmarks () const {
    cout << "size of ordered nodes is " << ordered_nodes.size() << endl;
    cout << "size of node set is " << nodes.size() << endl;
    LandmarkSet inferred_subgoals;

    for (auto n_it = nodes.begin(); n_it != nodes.end(); n_it++) {
        LandmarkNode *lmn = *n_it;
        ...
        for (int i = 0; i < l; i++) {
            ...
            if (g_predicates_methods_achieve.find((parse_fact_name (g_fact_names[var][val])).first) != g_predicates_methods_achieve.end()) {
                inferred_subgoals.insert (lmn);
                break;
            }
            // cout << (lmn->vars)[i] << " := " << (lmn->vals)[i] << " (" << (parse_fact_name (g_fact_names[var][val])).first << ")\t";
        }
        // cout << "disj:" << lmn->disjunctive << endl;
    }

    for (auto it = inferred_subgoals.begin(); it != inferred_subgoals.end(); it++) {
        LandmarkNode *lmn = *it;
        const int l = (lmn->vars).size();
        for (int i = 0; i < l; i++) {
            int var = (lmn->vars)[i], val = (lmn->vals)[i];
