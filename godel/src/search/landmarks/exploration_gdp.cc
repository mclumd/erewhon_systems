...
ExplorationGDP::ExplorationGDP(const Options &opts)
    : Heuristic(opts),
      did_write_overflow_warning(false) {
    cout << "Initializing ExplorationGDP..." << endl;
    ...
    for (int i = 0; i < (*g_current_goal).size(); i++) {
        int var = (*g_current_goal)[i].first, val = (*g_current_goal)[i].second;
...
ExplorationGDP::~ExplorationGDP() {
}

void ExplorationGDP::increase_cost(int &cost, int amount) {
    assert(cost >= 0);
    assert(amount >= 0);
    cost += amount;
...
void ExplorationGDP::write_overflow_warning() {
...
void ExplorationGDP::set_additional_goals(const std::vector<pair<int, int> > &add_goals) {
...
void ExplorationGDP::build_unary_operators(const Operator &op) {
...
void ExplorationGDP::setup_exploration_queue(const State &state,
...
void ExplorationGDP::relaxed_exploration(bool use_h_max = false, bool level_out = false) {
...
void ExplorationGDP::enqueue_if_necessary(ExProposition *prop, int cost, int depth,
                                       ExUnaryOperator *op,
                                       bool use_h_max) {
    assert(cost >= 0);
...
int ExplorationGDP::compute_hsp_add_heuristic() {
    int total_cost = 0;
...
int ExplorationGDP::compute_hsp_max_heuristic() {
...
int ExplorationGDP::get_lower_bound(const State &state) {
/* Note: this function is currently not used */
...
int ExplorationGDP::compute_ff_heuristic(const State &state) {
...
void ExplorationGDP::collect_relaxed_plan(ExProposition *goal,
...
void ExplorationGDP::compute_reachability_with_excludes(vector<vector<int> > &lvl_var,
    ...
    setup_exploration_queue(*g_current_state, excluded_props, excluded_ops, true);
...
void ExplorationGDP::prepare_heuristic_computation(const State &state, bool h_max = false) {
    setup_exploration_queue(state, h_max);
    relaxed_exploration(h_max);
    heuristic_recomputation_needed = false;
}

int ExplorationGDP::compute_heuristic(const State &state) {
...
void ExplorationGDP::collect_ha(ExProposition *goal,
                             RelaxedPlan &relaxed_plan, const State &state) {
...
bool is_landmark_gdp(vector<pair<int, int> > &landmarks, int var, int val) {
...
bool ExplorationGDP::plan_for_disj(vector<pair<int, int> > &landmarks,
                                const State &state) {
    relaxed_plan.clear();
    ...
    if (!landmarks.empty()) {
        ...
        for (int i = 0; i < termination_propositions.size(); i++) {
            ...
            if (prop_cost == -1 && is_landmark_gdp(landmarks, termination_propositions[i]->var,
                                               termination_propositions[i]->val)) {
                return false; // dead end
            }
            if (prop_cost < min_cost && is_landmark_gdp(landmarks, termination_propositions[i]->var,
                                                    termination_propositions[i]->val)) {
...
// function that cleans up the ExplorationGDP object for fresh use
void ExplorationGDP::cleanup () {
    for (int i = 0; i < propositions.size(); i++) {
        for (int j = 0; j < propositions[i].size(); j++) {
            ExProposition &expr = propositions[i][j];
            expr.is_goal_condition = false;
            expr.is_termination_condition = false;
            expr.marked = false;
            expr.reached_by = 0;
            expr.h_add_cost = -1;
            expr.h_max_cost = -1;
...
void ExplorationGDP::reset_goals (const goal_formula &gf) {
    for (int i = 0; i < gf.size(); i++) {
        int var = gf[i].first, val = gf[i].second;
        propositions[var][val].is_goal_condition = true;
        propositions[var][val].is_termination_condition = true;
        goal_propositions.push_back(&propositions[var][val]);
        termination_propositions.push_back(&propositions[var][val]);
