...
class GeneratorSwitch : public GeneratorBase {
    ...
    list<int> immediate_method_indices;
...
class GeneratorLeaf : public GeneratorBase {
    ...
    list<int> applicable_method_indices;
public:
    GeneratorLeaf(list<int> &operators, list<int> &methods);
    virtual void dump(string indent) const;
    virtual void generate_cpp_input(ofstream &outfile) const;
};
...
GeneratorSwitch::GeneratorSwitch(Variable *switch_variable,
    ...
    immediate_method_indices.swap(methods);
}
...
void GeneratorSwitch::dump(string indent) const {
    ...
    cout << indent << "immediately, operators:" << endl;
    ...
    cout << indent << "immediately, methods:" << endl;
    for (list<int>::const_iterator m_iter = immediate_method_indices.begin();
         m_iter != immediate_method_indices.end(); ++m_iter)
        cout << indent << *m_iter << endl;
...
void GeneratorSwitch::generate_cpp_input(ofstream &outfile) const {
    ...
    outfile << "check " << immediate_ops_indices.size() << " " << 
    immediate_method_indices.size() << endl;
    for (list<int>::const_iterator op_iter = immediate_ops_indices.begin();
         op_iter != immediate_ops_indices.end(); ++op_iter)
    ...
    for (list<int>::const_iterator m_iter = immediate_method_indices.begin();
        ...
        outfile << *m_iter << endl;
    for (int i = 0; i < switch_var->get_range(); i++) {
        //outfile << "--case "<<switch_var->get_name()<<" (Level " <<switch_var->get_level() <<
          //") has value " << i << ":" << endl;
        generator_for_value[i]->generate_cpp_input(outfile);
    }
    //outfile << "--always for " <<switch_var->get_name()<<" (Level " <<switch_var->get_level() << endl;
    default_generator->generate_cpp_input(outfile);
}

GeneratorLeaf::GeneratorLeaf(list<int> &ops, list<int> &methods) {
    applicable_ops_indices.swap(ops);
    applicable_method_indices.swap(methods);
}
...
void GeneratorLeaf::dump(string indent) const {
    ...
    for (list<int>::const_iterator op_iter = applicable_ops_indices.begin();
        ...
        cout << indent << *op_iter << endl;
    cout << indent << "methods:" << endl;
    for (list<int>::const_iterator m_iter = applicable_method_indices.begin();
         m_iter != applicable_method_indices.end(); ++m_iter)
        cout << indent << *m_iter << endl;
}

void GeneratorLeaf::generate_cpp_input(ofstream &outfile) const {
    outfile << "check " << applicable_ops_indices.size() << " " << 
    applicable_method_indices.size() << endl;
    for (list<int>::const_iterator op_iter = applicable_ops_indices.begin();
         op_iter != applicable_ops_indices.end(); ++op_iter)
        outfile << *op_iter << endl;
    for (list<int>::const_iterator m_iter = applicable_method_indices.begin();
...
SuccessorGenerator::SuccessorGenerator(const vector<Variable *> &variables,
    ...
    method_conditions.reserve(methods.size());
    list<int> all_operator_indices;
    list<int> all_method_indices;
    for (int i = 0; i < operators.size(); i++) {
        ...
        for (int j = 0; j < op->get_prevail().size(); j++) {
            ...
            // cout << "var is " << prev.var << " and val is " << prev.prev << endl;
        }
    ...
    for (int i = 0; i < methods.size(); i++) {
        ...
        Condition cond;
        for (int j = 0; j < m->get_prevail().size(); j++) {
            Method::VarVal prev = m->get_prevail()[j];
            cond.push_back(make_pair(prev.var, prev.val));
        }
        sort(cond.begin(), cond.end());
        all_method_indices.push_back(i);
        method_conditions.push_back(cond);
        next_condition_by_method.push_back(method_conditions.back().begin());
    }

    /*
    cout << "all_operator_indices is:" << endl;
    for (list<int>::iterator k = all_operator_indices.begin(); k != all_operator_indices.end(); k++) {
        cout << *k << endl;
    }


    cout << "all_method_indices is:" << endl;
    for (list<int>::iterator k = all_method_indices.begin(); k != all_method_indices.end(); k++) {
        cout << *k << endl;
    }
    */
    ...
    root = construct_recursive(0, all_operator_indices, all_method_indices);
}

GeneratorBase *SuccessorGenerator::construct_recursive(int switch_var_no,
                                                       list<int> &op_indices, 
                                                       list<int> &method_indices) {
    if (op_indices.empty() && method_indices.empty())
        return new GeneratorEmpty;

    while (true) {
        // Test if no further switch is necessary (or possible).
        if (switch_var_no == varOrder.size())
            return new GeneratorLeaf(op_indices, method_indices);
        ...
        // augmenting for methods
        vector<list<int> > methods_for_val_indices(number_of_children);
        list<int> default_method_indices;
        list<int> applicable_method_indices;

        
        bool all_ops_are_immediate = true;
        bool all_methods_are_immediate = true;
        bool var_is_interesting = false;
        bool var_is_interesting_by_method = false;
        ...
        // repeating above loop for methods ... 
        while (!method_indices.empty()) {
            int m_index = method_indices.front();
            method_indices.pop_front();
            assert(m_index >= 0 && m_index < next_condition_by_method.size());
            Condition::const_iterator &cond_iter = next_condition_by_method[m_index];
            assert(cond_iter - method_conditions[m_index].begin() >= 0);
            assert(cond_iter - method_conditions[m_index].begin() <= method_conditions[m_index].size());
            if (cond_iter == method_conditions[m_index].end()) {
                var_is_interesting_by_method = true;
                applicable_method_indices.push_back(m_index);
            } else {
                all_methods_are_immediate = false;
                Variable *var = cond_iter->first;
                int val = cond_iter->second;
                if (var == switch_var) {
                    var_is_interesting_by_method = true;
                    ++cond_iter;
                    methods_for_val_indices[val].push_back(m_index);
                } else {
                    default_method_indices.push_back(m_index);
                }
            }
        }
/*
        if (applicable_method_indices.size() + default_method_indices.size() > 0) {
            cout << "variable: " << switch_var->get_name() << endl;
            cout << "applicable methods:" << endl;
            for (list<int>::const_iterator m_iter = applicable_method_indices.begin();
                 m_iter != applicable_method_indices.end(); ++m_iter)
                cout << *m_iter << endl;
            cout << "default methods:" << endl;
            for (list<int>::const_iterator m_iter = default_method_indices.begin();
                 m_iter != default_method_indices.end(); ++m_iter)
                cout << *m_iter << endl;
        }
*/
        if (all_ops_are_immediate && all_methods_are_immediate) {
            return new GeneratorLeaf(applicable_ops_indices, applicable_method_indices);
        } else if (var_is_interesting || var_is_interesting_by_method) {
            vector<GeneratorBase *> gen_for_val;
            for (int j = 0; j < number_of_children; j++) {
                gen_for_val.push_back(construct_recursive(switch_var_no + 1,
                                                          ops_for_val_indices[j], 
                                                          methods_for_val_indices[j]));
            }
            GeneratorBase *default_sg = construct_recursive(switch_var_no + 1,
                                                            default_ops_indices, 
                                                            default_method_indices);
            return new GeneratorSwitch(switch_var, applicable_ops_indices, 
                                                   applicable_method_indices, 
                                                   gen_for_val, 
                                                   default_sg);
        } else {
            ...
            default_method_indices.swap(method_indices);
        }
    }
}
