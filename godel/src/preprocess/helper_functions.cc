...
void read_methods(istream &in, const vector<Variable *> &variables,
		  vector<Method> &methods) {
    cout << "entered read methods ... " << endl;
    int count;
    in >> count;
    for (int i = 0; i < count; i++)
        methods.push_back(Method(in, variables));
}
...
void read_preprocessed_problem_description(istream &in,
    ...
    read_methods(in, variables, methods);
}
...
void generate_cpp_input(bool /*solvable_in_poly_time*/,
    ...
    outfile << methods.size() << endl;
    for (int i = 0; i < methods.size(); i++)
        methods[i].generate_cpp_input(outfile);
