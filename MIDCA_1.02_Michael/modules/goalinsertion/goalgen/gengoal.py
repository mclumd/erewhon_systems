...
class XPGoalGen:
	...
	def gen_goals(self, verbose):
		self.histanalysis.verbose = verbose
		self.counter += 1
		if self.counter <= 1:
			return []
		...
		if s:
			time.sleep(.3)           # change this, do sockets properly, waiting for Meta-AQUA to send everything.            
			...
			if text != "None\n" and text != "" and text != None:
				...
				self.histanalysis.updatemetric(score)
				goalname = self.histanalysis.goalgen(frames)

				print "Here is goal which stat method produced: " + goalname
