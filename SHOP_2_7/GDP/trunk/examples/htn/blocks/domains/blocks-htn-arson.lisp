...
(defdomain blocks-normal
  (
    ...
    (:operator (!putoutfire ?h)       
		((not (notonfire ?h)))
                   
                   ((notonfire ?h))
    )
    
    (:operator (!apprehend ?i)
    	((arsonist ?i)
    	(free ?i))    			
    			((not (free ?i))
    )
...
;; The method for the top-layer task
    (:method (achieve-goals ?goals)
             ()
             ((assert-goals ?goals nil)(catch-arsonists)(move-block nil)(put-out-fires nil)))
    ...
    (:method (catch-arsonists)
    	((free ?x) (arsonist ?x) (goal (not (free ?x))))
    	((!apprehend ?x))
    )
    
    (:method (put-out-fires ?fireout)
    	
    	((need-to_put_out ?x))
    	((!putoutfire ?x) (put-out-fires (?x . ?fireout)))
    )
 ...
 (:method (put-out-fires ?fireout)    	
    ...
    (:- (need-to_put_out ?x)
    	((goal (notonfire ?x)) (not (notonfire ?x)))
    )
