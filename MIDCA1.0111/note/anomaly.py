...
class AnomalyDetector:
	...
	def __init__(self, world, mem, threshold = 0.5, size = None):
		...
		aDist.init(world)
		self.mem.update({midca_inst.MEM_ADIST: [aDist]})
		self.dimension = len(adistadapt.get_pred_names(world))
	
	def update(self, world):
		self.mem.update_all(midca_inst.MEM_ADIST, world)
	
	def anomalous(self):
		wrappers = self.mem.get(midca_inst.MEM_ADIST)
	...
	def clear(self):
		self.mem._clear(midca_inst.MEM_ADIST)
	...
	def __str__(self):
		s = ""
		n = 1
		for aDist in self.mem.get(midca_inst.MEM_ADIST):
...
class ADNoter:
	...
	def run(self, verbose = 2):
		...
		self.update(world)
		self.mem._add(midca_inst.MEM_ANOM, self.anomalous())
