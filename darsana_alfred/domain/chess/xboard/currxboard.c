 ...
 * $Id: currxboard.c,v 1.1 2004/06/17 22:04:59 darsana Exp $
 *
...
#define WIDTH_FUDGE 2
    ...
    //with modification to get input from socket
    for (;;) {
	...
	if (FD == PassiveSocket) {
	  ...
	  default:
		...
		instruction for the move */
	      ...
	      /*scanf("%s", move);*/
	      counter = 0;
	      /*is this the way to set it to an empty string?*/
	      while(counter < MSG_SIZ){
		move[counter] = buf[counter];
		counter++;
	      }
	      if (move != NULL && strcmp(move, "end") != 0) {
		ParseOneMove(move, TRUE, &moveType, &fromX, &fromY,
			     &toX, &toY, &promoChar);
