...
(GTYPE-OF OUTCOME-ALL-EARLY-DELIVERY :TOP-TYPE)
(GTYPE-OF OUTCOME-SOME-EARLY-DELIVERY OUTCOME-ALL-EARLY-DELIVERY)
(GTYPE-OF DELIVERED-IN-MORNING :TOP-TYPE)
(GTYPE-OF DELIVERED-IN-AFTERNOON DELIVERED-IN-MORNING)
...
(OPERATOR LOAD-TRUCK
  (params <obj> <truck> <loc> <city>)  ;; params are used for printing purposes only
  (preconds
   ((<obj> OBJECT)
    (<truck> TRUCK)
    (<city> CITY)
...
(OPERATOR FLY-AIRPLANE
  (params <airplane> <loc-from> <loc-to> <city-from> <city-to>)
  (preconds
   ((<airplane> AIRPLANE)
    (<city-from> CITY)
    (<city-to> CITY)
   ...
   (and 
    (loc-at <loc-from> <city-from>)
    (loc-at <loc-to> <city-to>)
...
(OPERATOR DELIVERED-EARLY
   (params <obj> <truck> <postoffice>); <city>)
   (preconds
    ((<obj> OBJECT)
     (<truck> TRUCK)
;     (<city> CITY)
     (<postoffice> POST-OFFICE))
    (and  (at-obj <obj> <postoffice>)


;	  (loc-at <postoffice> <city>)	 
	 (not-received <postoffice>)
	 (is-ready <truck>)
	 (not-delivered <obj>)    ;added June 12, 2001
	 
	 ))
   (effects ()
	    (
	     (del (not-received <postoffice>))
	     (del (is-ready <truck>))
	     (del (not-delivered <obj>)) ; added June 12, 2001
	     	     
	     (add (delivered-in-morning <truck> <obj> <postoffice>)))))


(OPERATOR DELIVERED-LATE
   (params <obj> <truck> <postoffice>); <city>)
   (preconds
    ((<obj> OBJECT)
     (<truck> TRUCK)
...
;	 (loc-at <postoffice> <city>)	 
   ...
   (effects ()
	    ((del (not-received <postoffice>))
	     (del (not-delivered <obj>)) ; added June 12, 2001
	     (add (delivered-in-afternoon <truck> <obj> <postoffice>)))))


;(OPERATOR ASSIGN-TRUCK
(Inference-Rule ASSIGN-TRUCK
   ;(mode lazy)
   (params <obj> <truck> <postoffice> <city>)
   (preconds
    ((<obj> OBJECT)
    ...
    (and
     (not-assigned <postoffice>)
     (loc-at <postoffice> <city>)
     (part-of <truck> <city>)
     (delivered-in-morning <truck> <obj> <postoffice>)
     ))
   (effects
    ()
    ((del (not-assigned <postoffice>))
     (add (delivered-in-morning <obj> ))
     ))
   )


;(OPERATOR ASSIGN-ANOTHER-TRUCK
(Inference-Rule ASSIGN-ANOTHER-TRUCK
   ;(mode lazy)
   (params <obj> <truck> <postoffice> <city>)
   (preconds
    ((<obj> OBJECT)
    ...
    (and
     ...
     (loc-at <postoffice> <city>)
     (part-of <truck> <city>)
     (delivered-in-afternoon <truck> <obj> <postoffice> )
     ))
   (effects
    ()
    (
     (del (not-assigned <postoffice>))
     (add (delivered-in-afternoon <obj>))
     ))
   )

(OPERATOR ENSURE-ALL-EARLY-DELIVERY
;(Inference-Rule ENSURE-ALL-EARLY-DELIVERY
   ; (mode eager) ;(mode lazy)
    (params <postoffice> <dummy-truck-unit>)		
    (preconds
     ((<postoffice> POST-OFFICE)
      (<dummy-truck-unit> TRUCK))
     (forall ((<obj> (AND OBJECT
			  (gen-from-pred
			   (at-obj <obj> <postoffice>) <postoffice>)))
	       ;(<dest-postoffice> (AND POST-OFFICE
	;			  (gen-from-pred
		;		   (not-received <dest-postoffice>))))	      
	      
	      )
	     (delivered-in-morning <obj>)
	    ))
    (effects
     ()
     ((add (outcome-all-early-delivery <dummy-truck-unit> <postoffice>))))
    )

;(Inference-Rule ENSURE-SOME-EARLY-DELIVERY
(OPERATOR ENSURE-SOME-EARLY-DELIVERY
  ;  (mode lazy)
    (params <postoffice>)		
    (preconds
     ((<postoffice> POST-OFFICE)
      ...
      (forall ((<obj> (AND OBJECT
	    ...
	    (delivered-in-afternoon <obj>)
	    ))
    (effects
     ()
     ((add (outcome-some-early-delivery <dummy-truck-unit> <postoffice>))))
    )


(INFERENCE-RULE IF-DELIVERED-IN-MORNING-THEN-DELIVERED-IN-AFTERNOON-TOO
    (mode lazy)
    (params <obj>)
    (preconds
     ((<obj> OBJECT)
...
(CONTROL-RULE REJECT-WHEN-NO-MORE-READY
	      (if (and
		   (current-goal (delivered-in-morning <truck> <obj>))
		   (type-of-object-gen <obj> OBJECT)
		   (false-in-state-forall-values (is-ready <truck>) (<truck> TRUCK))))
	      (then reject operator delivered-early))


(CONTROL-RULE SELECT-READY-TRUCK-BINDING-FOR-DELIVERED-EARLY
	      (if (and
		   (current-goal (delivered-in-morning <package>))
		   (type-of-object <city-unit> CITY)
		   (m-pred <city-unit> <package>)
		   (type-of-object <truck-unit> TRUCK)
		   (type-of-object <po-unit> POST-OFFICE)
		   (current-operator ASSIGN-TRUCK)
		   (true-in-state (is-ready <truck-unit>))
		   (true-in-state (not-received <po-unit>)
		   
		  ; (true-in-state (not-assigned <po-unit>)) ;added temporarily
				  
				  )))
	      (then select bindings ((<truck> . <truck-unit>) (<obj> . <package>) (<postoffice> . <po-unit>) (<city> . <city-unit>))))

(CONTROL-RULE SELECT-READY-TRUCK-BINDING-FOR-DELIVERED-LATE
	      (if (and
		   (current-goal (delivered-in-afternoon <package>))
		   (type-of-object <city-unit> CITY)
		   (m-pred <city-unit> <package>)		   
		   (type-of-object <truck-unit> TRUCK)
		   (type-of-object <po-unit> POST-OFFICE)
		   (current-operator ASSIGN-ANOTHER-TRUCK)
		   (true-in-state (is-ready <truck-unit>)
		   ;(true-in-state (not-assigned <po-unit>))
				  )))
	      (then select bindings ((<truck> . <truck-unit>) (<obj> . <package>) (<postoffice> . <po-unit>) (<city> . <city-unit>))))
...
;		   (current-operator ASSIGN-ANOTHER-TRUCK)
;		   (true-in-state (is-ready <truck-unit>))))
;	      (then select bindings ((<truck> . <truck-unit>) (<obj> . <package>))))
...
;;Added July 19, 2001
(defun m-pred (city pack)
  (eq city (at-po-or-airport pack))
  )
