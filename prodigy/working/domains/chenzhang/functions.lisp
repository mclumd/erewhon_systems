...
(defun in-truck-city-p (truck loc)
  (cond ((varp loc)
	 (if (varp truck)
	     (error "Can't have both truck and location variable in 
in-truck-city-p"))
...
(defun get-city-of-loc (loc)
  ...
  (let ((loc-hash (gethash 'loc-at
    ...
    (if result
	(elt (p4::literal-arguments result) 1)
	(error "~% CITY-OF-LOC: no entry for loc-at ~A in the state." 
loc))))
...
(defun in-truck (obj)
  ...
  (let* ((inside-truck-hash (gethash 'inside-truck
				     (p4::problem-space-assertion-hash
				      *current-problem-space*)))
	 (result (get-relevant-literal-in-hash-table obj 
inside-truck-hash)))
    (if result
