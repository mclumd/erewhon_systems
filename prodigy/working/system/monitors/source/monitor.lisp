...
;;; HISTORY
;;;
;;; Fixed bug in ok-stream fn. 27jul00 cox.
;;;
...
;;;
;;; The following overwrite is performed on function ok-stream from file
;;; monitors.lisp. The change is an option to disenable the sending of info to
;;; the tcl-tk UI. By adding the optional argument, we can test code in PRODIGY
;;; without the UI.
;;;
...
;;;
;;; Do NOT need the option. I added the test for null vars in the "and"
;;; conditions.
;;;
(defun ok-stream (&optional
		  (using-UI-p t))
  "Return t if UI running."
  (declare (special *tcl-send*))
  (and (boundp '*tcl-send*)
       ;;Replaced streamp because *tcl-send* could be bound to nil [27jul00 cox]
;       (streamp *tcl-send*)
       (not (null *tcl-send*))
