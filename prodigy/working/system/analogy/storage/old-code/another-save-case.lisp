...
(defun store-solution (ofile solution-path)
  (let* ((plan (mapcan
		...
		solution-path)))
    (format ofile "~S" (mapcar #'get-op-info plan))))

(defun get-op-info (inst-op)
  (let ((visible-inst-op (get-visible-inst-op inst-op))
	(guidance 
	 (getf (p4::nexus-plist
		(p4::instantiated-op-binding-node-back-pointer inst-op))
	       :guided)))
    (if guidance 
	(list visible-inst-op guidance)
      (list visible-inst-op))))

(defvar *store-static-state* t)

(defun store-state-goal (state-or-goal ofile lits)
  (cond
   ((null lits) nil)
   ((and (= (length lits) 1)
	 (eq (caar lits) 'and))
    (if (and (eq state-or-goal 'state)
	     (not *store-static-state*))
	(store-not-static-state ofile (cdar lits))
        (store-state-goal state-or-goal ofile (cdar lits))))
   (t
    (format ofile "~S" lits))))

(defun store-not-static-state (ofile lits)
  (let ((dynamic-state nil))
    (dolist (lit lits)
      (unless
	  (member (car lit) (p4::problem-space-static-preds *current-problem-space*))
	(push lit dynamic-state)))
    (format ofile "~S" dynamic-state)))
...
(defun write-case-header-to-file
  ...
  (let ((filename (concatenate 'string *problem-path*
	...
	(format t "~% ~S" filename))
      ...
      (store-state-goal 'goal ofile (cdr (p4::problem-goal (current-problem))))
      (format ofile ")")
      (format ofile "~%(setf initial-state '")
      (store-state-goal 'state ofile (cdr (p4::problem-state (current-problem))))
      (format ofile ")")
