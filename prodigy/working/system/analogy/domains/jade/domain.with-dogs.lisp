...
(OPERATOR SECURE-PERIMETER
  ...
  (preconds
   ...
   (and 
    (loc-at <perimeter> <loc>)
;Temporary [cox 4mar98]
    (~ (is-enemy <infantry-unit>))
...
;;;Deleted the following three temporarily. Change back soon!! [cox 4mar98]
	(~ (is-enemy <internal-security-force>))
	(~ (is-enemy <external-security-force1>))
	(~ (is-enemy <external-security-force2>))
...
(CONTROL-RULE SELECT-ISOLATE-BATTLEFIELD-BINDINGS
  ...
  (then select bindings ((<air-unit> . <a-unit>) (<naval-unit> . <n-unit>)))
  )
...
;;; Used to be 3 rules for secure-stronghold bindings. But only one will fire.
#|
