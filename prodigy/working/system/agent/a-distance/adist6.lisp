...
(defun make-wp (data p o n)
  (make-windowPair :w1 () 
		   :w2 () 
		   :i1 (create-integer-intervals
			;;create-proportional-intervals  
			(copy-list data) ;Not sure we need copy-list [mcox]
			;;p o
			)
		   :i2 (create-integer-intervals 
			;;create-proportional-intervals  
			(copy-list data)  ;Not sure we need copy-list [mcox]
			;;p o
			) 
...
#|
     ...
     for i in range(min(data), max(data) + 1):
         ad.add(Interval(i - 0.5, i + 0.5))
     print ad
     |#

;;; 
;;; Function range returns a list of sequential integers from min to max.
;;; 
(defun range (min
	      max
	      &optional 
	      current)
  "Return a list with elements min .. max"
  (cond ((> min max)
	 nil)
	((= min max)
	 (list max))
...
;;; Implements the python code above.
(defun create-integer-intervals (data
				 &aux
				 (sorted-data
				  (sort data #'<))
				 result-list)
  (dolist (i (range (first sorted-data)
		    (first (last 
			    sorted-data)))
	    (reverse result-list))
    (setf result-list
      (cons (create-interval
	     (- i 0.5) 
	     (+ i 0.5))
	    result-list))
    )
...
(defun epsilon-helper (data 
  ...
  (let* ((shuffled 
	 ...
	 ;; Is P and o swapped i.e., should they be passed as 0.5 .05?
	 ;; [mcox 24apr12]
...
;;;helper for find changes
;;; variant of change-helper that returns a list of 0/1 representing
;;;changes at each step [mpaisner 9/10/12]
;;;changed again to return a-distance values [mpaisner 10/31/12] 
(defun changes-helper (wp data epsilon)
  (if data
      (let ((wpNew (add-data wp (first data))))
	;; Should initialize wp and remove the window-full test 
	;; as with largest-epsilon [mcox 7apr12]
	(if (window-full (windowPair-w1 wpNew) (windowPair-n wpNew))
	    (cons (a-dist wpNew t) (changes-helper wpNew (rest data) epsilon))
            (changes-helper wpNew (rest data) epsilon)))
    nil))
...
;;;modification of find-changes that returns list of 0/1 for changes at
;;;each step [mpaisner 9/10/12] 
(defun find-changes (wp data epsilon)
   (changes-helper wp data epsilon)  
  )
...
;;; begins. Consider that each plan of n vectors will be converted to
;;; a sequence of n-1 differences between vectors (actually on a
;;; predicate basis rath erhtna on a vector basis). Thus we must count
;;; plans one less than their length. So if the 3rd plan in the
;;; sequence is anomalous, then the step at which the anomaly begins
;;; is (- (length (first seq)) 1) + (- (length (second seq)) 1) + 1 or
;;; (plan2step seq 3).
;;; 
...
;;; x)), step2plan will return 3. Note that the length of each plan is
;;; counted one less than its actual length. See comment in plan2step
;;; above.
;;; 
