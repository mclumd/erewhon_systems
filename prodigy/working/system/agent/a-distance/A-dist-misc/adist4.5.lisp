...
(defun alpha-helper (data 
  ...
  (let* ((shuffled 
    ...
    (if (= num-iterations 0)
	nil
      (cons (largest-alpha local-wp	;Use copy passed [mcox 5apr12]
...
;;; 
(defun compute-alpha (data p num-iterations
		      &optional
		      debug)
  (let ((maxDists 
...
(defun plan2step (sequence plan-num)
  (cond ((null sequence)		;If t something is wrong
	 0)
	((eql 0 plan-num)
	 0)
	(t 
	 (+ (length (first sequence))
	    (plan2step (rest sequence)
		       (- plan-num 1)))))
  )
