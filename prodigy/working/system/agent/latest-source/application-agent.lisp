...
;;;; HISTORY
;;;; 
;;;; Changed :remote-host string naming host from djembe to localhost in
;;;; function establish-active-agent-connection.  Deleted extra copy of
;;;; function definition that used global variable *myhost*. [mcox 21dec05]
;;;; 
...
(defun establish-active-agent-connection (port-to-connect)
  ...
  (setf *active-socket* 
    (socket:make-socket :remote-host "localhost" :remote-port port-to-connect))))
  )
...
(defun request-and-return-plan (&key
  ...
  (if (not ack-time-out)
	 ...
	 (let 
	   ...
	   (when (eq (first temp) 'inform)
	     (setf temp 
	       (process-inform socket :request temp)))
...
(defvar *window-stack* nil "For Jeremy's CONDUIT")
