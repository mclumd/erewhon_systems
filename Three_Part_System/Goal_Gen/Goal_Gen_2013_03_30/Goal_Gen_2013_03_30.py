...
class GoalGen20130330(GoalGen):
    ...
    def _classify(self, blockset):
        ...
        for a in range(len(blockset)):
            A = blockset[a]
            if not (A.blocktype == Block.TRIANGLE and A.clear):                         # triangle(A) and clear(A)
                continue

            for b in range(len(blockset)):
                B = blockset[b]
                if not (A.on == B and B.blocktype == Block.TABLE):                      # on(A,B) and table(B)
                    continue
                else:
                    prediction = 3
                    return prediction
