...
class Parser:
    ...
    def makeframegraph(self, text):
        ...
        for l in lines:
            ...
            elif l[0:20] == "For the state frame ":
                framename = l[20:-1]
                if not framename in frames:
                    frames[framename] = Frame(framename, isstate=True, iscenter=centerflag)
                    centerflag = False

            elif l[0:30] == "For the physical-object frame ":
                framename = l[30:-1]
                if not framename in frames:
                    frames[framename] = Frame(framename, isstate=False, iscenter=centerflag)
                    centerflag = False

            elif l[0:30] == "For the abstract-object frame ":
                framename = l[30:-1]
                if not framename in frames:
                    frames[framename] = Frame(framename, isstate=False, iscenter=centerflag)
                    centerflag = False

            elif l[0:22] == "For the process frame ":
                framename = l[22:-1]
                if not framename in frames:
                    frames[framename] = Frame(framename, isstate=False, iscenter=centerflag)
                    centerflag = False

            elif "For the" in l and "frame" in l:
                print "UNEXPECTED FRAME TYPE (!!!!!!!), l: " + l
                sys.exit(0)
        ...
        for l_index in range(len(lines)):
            ...
            if len(l) > 14 and l[0:14] == "For the frame ":
                curframe = l[14:-1]
            elif len(l) > 20 and l[0:20] == "For the state frame ":
                curframe = l[20:-1]
            elif len(l) > 30 and l[0:30] == "For the physical-object frame ":
                curframe = l[30:-1]
            elif len(l) > 30 and l[0:30] == "For the abstract-object frame ":
                curframe = l[30:-1]
            elif len(l) > 22 and l[0:22] == "For the process frame ":
                curframe = l[22:-1]
