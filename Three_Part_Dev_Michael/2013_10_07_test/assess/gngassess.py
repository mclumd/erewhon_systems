...
class AnomalyAnalyzer:
	...
	def init_gng(self):
		self.trace = self.mem.get(midca_inst.MEM_ADIST)[0].trace
	...
	def run(self, verbose = 2):
		...
		while len(self.trace.values) > self.numUpdates:
			...
			self.numUpdates += 1
		if self.mem.get(midca_inst.MEM_ANOM)[-1]:
			self.mem._add(midca_inst.MEM_NODES, (self.last_node(), self.numUpdates))
			self.mem._add(midca_inst.MEM_ANOM_TYPE, self.anomaly_type())
			if verbose >= 1:
