...
class MemType:
	...
	def __init__(self, parent, fixedAttrs, variableAttrs, optionalAttrs):
		self.parent = parent
		self.fixedAttrs = fixedAttrs
		self.variableAttrs = variableAttrs
		self.optionalAttrs = optionalAttrs


class Memory:
	
	#1) is_a -> is of type
	#2) is -> has the characteristic defined by
	#3) has -> has as part of itself - not possession.
