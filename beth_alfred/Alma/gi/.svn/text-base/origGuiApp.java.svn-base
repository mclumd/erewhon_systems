...
public class GuiApp extends Frame implements AbGui{
	...
	while(index < args.length){
		...
		if(args[index + 1].compareTo("true")==0) autoStart = true;
		index = index + 2;
		continue;
	    } // almaArgs
		...
		historyInputURLName = args[index + 1];
		index = index + 2;
		continue;
	    } // almaArgs
	    ...
	    System.err.println("Argument " + argName + " is unknown. Ignored");
	    index = index + 1;
	} // while
