...
fif(and(end_of_parse(Utt),
	eval_bound(mult_gather_all([links(Utt,_,_,_,_), 
				   value_of(Utt,_,_)],
				   Asserts), [Utt])),
conclusion(call(ac_process_parse(Utt), Asserts, Utt))).    


/*use context to parse*/
fif(and(done(ac_process_parse(Utt),_,Utt),
	and(eval_bound(\+ pos_int_u(utt_struct(Utt,_,_,_)), [Utt]),
