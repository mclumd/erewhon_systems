 ...
 * $Id: xboard.c,v 1.1 2004/06/17 22:05:08 darsana Exp $
 *
...
static void DragPieceBegin P((int x, int y));  /* Jay: piece move starts with mouse click*/
static void DragPieceMove P((int x, int y));   /*Checking mouse drag*/
static void DragPieceEnd P((int x, int y));    /*End point of mouse click*/
     ...
     char *argv[];
{
    ...
    if(tcp_create_listener(0, &Port, &Host, &PassiveSocket)) 
      exit(errno);
    
    if(tcp_address_to_file(argv[1], Port, Host)) 
      exit(errno);
    ...
    /*dont think I need to have this anymore since argc has to be 2*/
    ...
    //Some modifications to preferences - Randy
...
#define WIDTH_FUDGE 2
    ...
    //Paolo's Venerable Code 
    //with modification to get input from socket from SHIVANGI
    strcpy(resultOfGame,"");
    promoChar='q';
					...
					else
					{
	  					...
	  					switch (n) 
	  					{
							...
							default:
	    						...
	    						{
	      							write(1, "\n", 1);
	    							/*The instruction is now in buf and the chess program can use this
										instruction for the move */
	      							printf("Please enter the next move: ");
	      							counter = 0;
								while(counter < MSG_SIZ)
								{
									move[counter] = buf[counter];
									counter++;
								}
								//want to modify the text from Alfred in here...
	...
	if(gameMode!=EndOfGame)
	{
		      	...
		      	if (move != NULL) 
			{
	      			...
	      			{
					printf("Moving: %s\n",move);
					ParseOneMove(move, moveCounter, &moveType, &fromX, &fromY,&toX, &toY, &promoChar);
					...
					CallFlagEvent();
		      		...
		      		/*
				Jay's "venerable" code...  Doesn't seem to work right.
				...
				*/
				...
				{
					printf("The move is out of bounds.\n",move[0],move[1],move[2],move[3]);
				}
			}
			else
			{
				printf("The move syntax is seriously invalid.\n");
			}
		}
		else
		{
			printf("This move syntax is invalid.\n");
		}
	    }/*end for*/
    }
...
/*
 * JAY: this function maps an event (mouse-click) to the board checking
 * if the move was within boundary.
 */ 
...
/* Jay: Start the Move using Mouse-Click*/
